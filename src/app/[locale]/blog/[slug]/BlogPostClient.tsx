    ...
    return (
      <main className="min-h-screen text-slate-900 dark:text-slate-100">
        ...
        <section className="px-4 pb-16 pt-6 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl rounded-[2rem] border border-slate-200 bg-white p-10 text-center shadow-3xl dark:border-slate-700 dark:bg-slate-800">
            ...
            <p className="mt-3 text-slate-600 dark:text-slate-400">{blogPage.notFoundDescription}</p>
            <Link
              href="/blog"
      ...
      <section className="px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pt-10">
        <div className="mx-auto max-w-5xl">
          <motion.header
            ...
            className="mb-8"
          >
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-950 dark:text-slate-400 dark:hover:text-slate-200">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
            ...
            <h1 className="mt-4 text-balance text-2xl font-semibold tracking-tight text-slate-950 dark:text-slate-50 sm:text-4xl md:text-5xl">
              {post.title}
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600 dark:text-slate-400 sm:text-lg sm:leading-8">{post.excerpt}</p>
          ...
          >
            ...
            >
              ...
              <div className="absolute right-4 top-4 rounded-full bg-white/88 px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm dark:bg-slate-800/88 dark:text-slate-300">
                Click to preview
              </div>
            </button>

            <div className="grid gap-8 p-7 sm:p-10 lg:grid-cols-[0.65fr_1.35fr]">
              <aside className="rounded-[1.5rem] border border-slate-200 bg-slate-50/90 p-6 dark:border-slate-700 dark:bg-slate-800/50">
                ...
                {post.content.map((item, index) => {
                  ...
                  return (
                    <div key={index} className="space-y-5">
                      {typeof item === 'string' && (
                        <p className="text-base leading-8 text-slate-700 dark:text-slate-300 md:text-lg">{item}</p>
                      )}
                      
                      {isVideo && (
                        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white/80 shadow-sm dark:border-slate-700 dark:bg-slate-800/80">
                      ...
                      {inlineImages[index + 1]?.map((image) => (
                        <button
                          ...
                          className="block w-full cursor-zoom-in overflow-hidden rounded-2xl border border-slate-200 bg-white/80 text-left shadow-sm dark:border-slate-700 dark:bg-slate-800/80"
      ...
      <section className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          >
            ...
            <Link href="/blog" className="text-sm font-semibold text-slate-600 hover:text-slate-950 dark:text-slate-400 dark:hover:text-slate-200">
              {blogPage.viewAll}
            </Link>
          </motion.div>
            ...
            {relatedPosts.map((relatedPost, index) => (
              ...
              >
                <Link href={`/blog/${relatedPost.slug}`} className="block">
                  ...
                  <div className="p-6">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">{relatedPost.publishedAt}</p>
                    <h3 className="mt-2 text-xl font-semibold tracking-tight text-slate-950 dark:text-slate-50 sm:text-2xl">{relatedPost.title}</h3>
                    <p className="mt-3 text-sm leading-7 text-slate-600 dark:text-slate-400">{relatedPost.excerpt}</p>
                  </div>
                </Link>
              </motion.article>
            ))}
